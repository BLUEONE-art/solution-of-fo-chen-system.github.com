...
%-------------------------------------------------------------------------------------------------------%
clear;clc;
I=imread('../原始、加密、解密图片/lena.jpg','jpg');         %读取图像信息
...
SUM=M*N;
%% 2.产生Logistic混沌序列
u=3.9999;     %Logistic参数μ，自定为3.99，作为密钥1
x0=(sum(I1(:))+sum(I2(:)))/(255*SUM*2);     %计算得出Logistic初值x0，作为密钥2，这里是根据原始RGB彩色空间图像进行算的密钥，只跟明文相关
...
rate1=1;
YS1=ones(t,t);
for c=(t-rate1+1):rate
    for d=(t-rate1+1):(rate)
        YS1(c,d)=0;
    end
end
% disp(YS1);
...
fun = @(block_struct) T'*block_struct.data*T;       %Matlab建议用blockproc函数代替blkproc函数
K11 = blockproc(J1,[t t],fun);              %DCT逆变换，重构图像
K11 = im2uint8(K11);
...
Q1_R=DNA_bian(fenkuai(t,J1,1),X(1));%这里对转换量化后的彩色空间Y分量图像进行DNA编码
...
imwrite(Q_jiami_RGB,'../原始、加密、解密图片/加密后的lena.png','png'); 
figure;imshow(Q_jiami_RGB);title('加密后图片');
